...
var NoiseEnabled = os.Getenv("OSS_NOISE_DISABLED_EXPERIMENTAL") != "true"
...
func Dial(ctx context.Context, dialer rpc.Dialer, nodeURL oss.NodeURL, config Config) (*Client, error) {
...
func DialReplaySafe(ctx context.Context, dialer rpc.Dialer, nodeURL oss.NodeURL, config Config) (*Client, error) {
...
func (client *Client) Retain(ctx context.Context, req *pb.RetainRequest) (err error) {
	...
	// but it requires more changes in xlr8d.io/oss/storagenode first
...
func (client *Client) NodeURL() oss.NodeURL { return client.nodeURL }
