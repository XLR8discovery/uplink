...
import (
	...
	"xlr8d.io/common/errs2"
	"xlr8d.io/common/pb"
	"xlr8d.io/common/ranger"
	"xlr8d.io/common/rpc"
	"xlr8d.io/common/oss"
	"xlr8d.io/uplink/private/eestream"
	"xlr8d.io/uplink/private/piecestore"
)
...
type Client interface {
	PutSingleResult(ctx context.Context, limits []*pb.AddressedOrderLimit, privateKey oss.PiecePrivateKey, rs eestream.RedundancyStrategy, data io.Reader) (results []*pb.SegmentPieceUploadResult, err error)
	Get(ctx context.Context, limits []*pb.AddressedOrderLimit, privateKey oss.PiecePrivateKey, es eestream.ErasureScheme, size int64) (ranger.Ranger, error)
	GetWithOptions(ctx context.Context, limits []*pb.AddressedOrderLimit, privateKey oss.PiecePrivateKey, es eestream.ErasureScheme, size int64, opts GetOptions) (ranger.Ranger, error)
	...
	PutPiece(ctx, parent context.Context, limit *pb.AddressedOrderLimit, privateKey oss.PiecePrivateKey, data io.ReadCloser) (hash *pb.PieceHash, id *struct{}, err error)
}

type dialPiecestoreFunc func(context.Context, oss.NodeURL) (*piecestore.Client, error)
...
func (ec *ecClient) dialPiecestore(ctx context.Context, n oss.NodeURL) (*piecestore.Client, error) {
...
func (ec *ecClient) PutSingleResult(ctx context.Context, limits []*pb.AddressedOrderLimit, privateKey oss.PiecePrivateKey, rs eestream.RedundancyStrategy, data io.Reader) (results []*pb.SegmentPieceUploadResult, err error) {
...
func (ec *ecClient) put(ctx context.Context, limits []*pb.AddressedOrderLimit, privateKey oss.PiecePrivateKey, rs eestream.RedundancyStrategy, data io.Reader, expiration time.Time) (successfulNodes []*pb.Node, successfulHashes []*pb.PieceHash, err error) {
	defer mon.Task()(&ctx,
...
func (ec *ecClient) PutPiece(ctx, parent context.Context, limit *pb.AddressedOrderLimit, privateKey oss.PiecePrivateKey, data io.ReadCloser) (hash *pb.PieceHash, deprecated *struct{}, err error) {
	if limit == nil {
...
func (ec *ecClient) Get(ctx context.Context, limits []*pb.AddressedOrderLimit, privateKey oss.PiecePrivateKey, es eestream.ErasureScheme, size int64) (rr ranger.Ranger, err error) {
	return ec.GetWithOptions(ctx, limits, privateKey, es, size, GetOptions{})
}

func (ec *ecClient) GetWithOptions(ctx context.Context, limits []*pb.AddressedOrderLimit, privateKey oss.PiecePrivateKey, es eestream.ErasureScheme, size int64, opts GetOptions) (rr ranger.Ranger, err error) {
...
func unique(limits []*pb.AddressedOrderLimit) bool {
	...
	ids := make(oss.NodeIDList, len(limits))
	for i, addressedLimit := range limits {
		if addressedLimit != nil {
	...
	for i := 1; i < len(ids); i++ {
		if ids[i] != (oss.NodeID{}) && ids[i] == ids[i-1] {
			return false
		}
	}
...
type lazyPieceRanger struct {
	...
	privateKey     oss.PiecePrivateKey
	size           int64
}
...
func limitToNodeURL(limit *pb.AddressedOrderLimit) oss.NodeURL {
	return (&pb.Node{
