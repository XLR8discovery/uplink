...
import (
	...
	"xlr8d.io/uplink/private/eestream/scheduler"
	"xlr8d.io/uplink/private/metaclient"
	"xlr8d.io/uplink/private/stalldetection"
	"xlr8d.io/uplink/private/storage/streams/splitter"
)
...
var (
	...
	fakeSegmentID  = oss.SegmentID{0xFF}
	...
	fakeSegmentInfo = &splitter.SegmentInfo{
		Encryption:    metaclient.SegmentEncryption{EncryptedKeyNonce: oss.Nonce{0: 1}},
		PlainSize:     123,
		EncryptedSize: 456,
	}
)
...
type nodeKind oss.NodeID

func isNodeKind(nodeID oss.NodeID, kind nodeKind) bool {
...
func makeNodeID(i int, kind nodeKind) oss.NodeID {
	nodeID := oss.NodeID(kind)
	nodeID[1] = byte(i)
	return nodeID
}

func fastNodeID(i int) oss.NodeID {
	return makeNodeID(i, fastKind)
}

func makeNodeIDs(kinds ...nodeKind) []oss.NodeID {
	var nodes []oss.NodeID
...
func pieceID(num int) oss.PieceID {
	return oss.PieceID{0: byte(num)}
}
...
func (fakeLimitsExchanger) ExchangeLimits(ctx context.Context, segmentID oss.SegmentID, pieceNumbers []int) (oss.SegmentID, []*pb.AddressedOrderLimit, error) {
	return nil, nil, errors.New("exchanges disallowed for test")
}

type fakePiecePutter struct{}

func (fakePiecePutter) PutPiece(longTailCtx, uploadCtx context.Context, limit *pb.AddressedOrderLimit, privateKey oss.PiecePrivateKey, data io.ReadCloser) (hash *pb.PieceHash, deprecated *struct{}, err error) {
...
func mustNewPiecePrivateKey() oss.PiecePrivateKey {
	pk, err := oss.PiecePrivateKeyFromBytes(bytes.Repeat([]byte{1}, 64))
	if err != nil {
		panic(err)
	}
	return pk
...
func mustNewRedundancyStrategy() eestream.RedundancyStrategy {
	rs, err := eestream.NewRedundancyStrategyFromOSS(oss.RedundancyScheme{
		Algorithm:      oss.ReedSolomon,
		ShareSize:      64,
		RequiredShares: 1,
		RepairShares:   2,
