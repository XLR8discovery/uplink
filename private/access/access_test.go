...
import (
	...
	privateAccess "xlr8d.io/uplink/private/access"
)
...
func newAccessGrant() (access *uplink.Access, secret []byte, err error) {
	...
	encAccess := grant.NewEncryptionAccessWithDefaultKey(&oss.Key{})
	encAccess.SetDefaultPathCipher(oss.EncNull)
	encAccess.LimitTo(apiKey)

	grantAccess := grant.Access{
		SatelliteAddress: oss.NodeURL{ID: testrand.NodeID()}.String(),
		APIKey:           apiKey,
		EncAccess:        encAccess,
	}
