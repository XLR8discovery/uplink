...
import (
	...
	"xlr8d.io/common/readcloser"
	"xlr8d.io/common/oss"
	"xlr8d.io/common/testcontext"
...
func TestRSRanger(t *testing.T) {
	...
	encKey := oss.Key(sha256.Sum256([]byte("the secret key")))
	var firstNonce oss.Nonce
	const stripesPerBlock = 2
	blockSize := stripesPerBlock * rs.StripeSize()
	encrypter, err := encryption.NewEncrypter(oss.EncAESGCM, &encKey, &firstNonce, blockSize)
	if err != nil {
		t.Fatal(err)
	}
	...
	decrypter, err := encryption.NewDecrypter(oss.EncAESGCM, &encKey, &firstNonce, blockSize)
	if err != nil {
		t.Fatal(err)
	}
