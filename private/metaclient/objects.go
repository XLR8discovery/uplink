...
func (db *DB) UpdateObjectMetadata(ctx context.Context, bucket, key string, newMetadata map[string]string, etag []byte, setETag bool) (err error) {
	...
	var encryptedKeyNonce oss.Nonce
	...
	encryptedStreamInfo, err := encryption.Encrypt(streamInfo, encryptionParameters.CipherSuite, &metadataKey, &oss.Nonce{})
	if err != nil {
		return err
	}
	...
	encryptedETag, err := encryption.Encrypt(etag, encryptionParameters.CipherSuite, &metadataKey, &oss.Nonce{1})
	if err != nil {
		return err
	}
...
type DeleteObjectsResultItem struct {
	...
	Status oss.DeleteObjectsStatus
}
		...
		if err != nil {
			if encryption.ErrMissingEncryptionBase.Has(err) {
				resultItems = append(resultItems, DeleteObjectsResultItem{
					...
					Status:           oss.DeleteObjectsStatusUnauthorized,
				})
				continue
			}
			return nil, err
...
func (db *DB) ListObjects(ctx context.Context, bucket string, options ListOptions) (list ObjectList, err error) {
	...
	if options.Delimiter != "" {
		if options.Delimiter != "/" && pi.Cipher != oss.EncNull {
	...
	if pi.Cipher == oss.EncNull {
		encPrefix = []byte(options.Prefix)
	}

	if options.Prefix != "" && !strings.HasSuffix(options.Prefix, "/") && pi.Cipher != oss.EncNull {
	...
	for {
		items, more, err := db.metainfo.ListObjects(ctx, ListObjectsParams{
			...
			ArbitraryPrefix:       pi.Cipher == oss.EncNull,
		})
		if err != nil {
			return ObjectList{}, errClass.Wrap(err)
		}
...
func (db *DB) objectsFromRawObjectList(ctx context.Context, items []RawObjectListItem, pi *encryption.PrefixInfo, delimiter []byte) (objectList []Object, err error) {
	...
	for _, item := range items {
		...
		if pi.Cipher == oss.EncNull && pi.PathEnc.Valid() {
...
func (db *DB) CommitObject(ctx context.Context, bucket, key, uploadID string, customMetadata map[string]string, etag []byte, encryptionParameters oss.EncryptionParameters, ifNoneMatch []string) (info Object, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	id, err := oss.StreamIDFromBytes(decodedStreamID)
	if err != nil {
		return Object{}, err
	}
...
func (db *DB) fillMetadata(bucket, key string, id oss.StreamID, metadata map[string]string, etag []byte, encryptionParameters oss.EncryptionParameters) (CommitObjectParams, error) {
...
// ObjectFromRawObjectItem converts RawObjectItem into oss.Object struct.
func (db *DB) ObjectFromRawObjectItem(ctx context.Context, bucket, key string, objectInfo RawObjectItem) (Object, error) {
	...
	if object.Stream.EncryptionParameters.CipherSuite == oss.EncUnspecified {
		object.Stream.EncryptionParameters = oss.EncryptionParameters{
			CipherSuite: oss.CipherSuite(streamMeta.EncryptionType),
...
func (db *DB) objectFromRawObjectListItem(bucket string, path oss.Path, listItem RawObjectListItem, stream *pb.StreamInfo, streamMeta pb.StreamMeta, etag []byte) (Object, error) {
	object := Object{
...
func (db *DB) typedDecryptStreamInfo(ctx context.Context, bucket string, unencryptedKey paths.Unencrypted, encryptedUserData EncryptedUserData) (_ *pb.StreamInfo, _ pb.StreamMeta, etag []byte, err error) {
	...
	cipher := oss.CipherSuite(streamMeta.EncryptionType)
	...
	streamInfo, err := encryption.Decrypt(streamMeta.EncryptedStreamInfo, cipher, contentKey, &oss.Nonce{})
	if err != nil {
		return nil, pb.StreamMeta{}, nil, err
	}
	...
	if len(encryptedUserData.EncryptedETag) > 0 {
		etag, err = encryption.Decrypt(encryptedUserData.EncryptedETag, cipher, contentKey, &oss.Nonce{1})
		if err != nil {
			return nil, pb.StreamMeta{}, nil, err
		}
	}
...
func getEncryptedKeyAndNonce(metadataKey []byte, metadataNonce oss.Nonce, m *pb.SegmentMeta) (oss.EncryptedPrivateKey, *oss.Nonce) {
	if len(metadataKey) > 0 {
		return oss.EncryptedPrivateKey(metadataKey), &metadataNonce
	}
