...
type LimitsExchanger interface {
	ExchangeLimits(ctx context.Context, segmentID oss.SegmentID, pieceNumbers []int) (oss.SegmentID, []*pb.AddressedOrderLimit, error)
}
...
func NewManager(exchanger LimitsExchanger, pieceReader PieceReader, segmentID oss.SegmentID, limits []*pb.AddressedOrderLimit) *Manager {
...
func (mgr *Manager) Results() (oss.SegmentID, []*pb.SegmentPieceUploadResult) {
	mgr.mu.Lock()
