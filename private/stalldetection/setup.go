...
func ConfigFromEnv() (config *Config) {
	...
	return &Config{
		BaseUploads:        getEnvInt("OSS_STALL_DETECTION_BASE_UPLOADS", defaultConfig.BaseUploads),
		Factor:             getEnvInt("OSS_STALL_DETECTION_FACTOR", defaultConfig.Factor),
		MinStallDuration:   getEnvDuration("OSS_STALL_DETECTION_MIN_DURATION", defaultConfig.MinStallDuration),
		DynamicBaseUploads: strings.ToLower(os.Getenv("OSS_STALL_DETECTION_DYNAMIC_BASE")) == "true",
	}
}
...
func isEnabled() bool {
	return strings.ToLower(os.Getenv("OSS_STALL_DETECTION_ENABLED")) == "true"
}
